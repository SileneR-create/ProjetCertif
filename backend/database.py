...
def db_get_user_stats() -> dict:
    ...
    try:
        ...
        total_admins = session.query(User).filter_by(is_admin=1).count()
...
def db_get_admin_search_history() -> list:
    """Récupère l'historique de recherche global pour l'administration."""
    session = get_session()
    try:
